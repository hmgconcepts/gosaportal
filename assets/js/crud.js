...
const CRUD = {
  ...
  */
  async renderList(moduleId, options = {}) {
    ...
    let query = d.generic
      ? this.sb.from(listTable).select('*').eq('module', d.module).order('created_at', { ascending: false }).limit(500)
      : this.sb.from(listTable).select('*').order('created_at', { ascending: false }).limit(500);
  ...
  async save(moduleId, id) {
    ...
    if (!id && window.SC_PROFILE && SC_PROFILE.id && !(window.App && App.isAdminRole && App.isAdminRole(App.currentRole))) {
      ...
      if (ownedTables.includes(d.table)) {
        ...
        if (!payload.teacher && SC_PROFILE.full_name) payload.teacher = SC_PROFILE.full_name;
      }
    }
